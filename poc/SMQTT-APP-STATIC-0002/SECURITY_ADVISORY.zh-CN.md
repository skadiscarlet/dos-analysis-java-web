# Security Advisory PoC：SMQTT retained MQTT messages map 可由匿名 retain publish 触发 OOM

## 摘要

SMQTT 默认匿名 MQTT 客户端可发送 retain=true 的 PUBLISH。不同 topic 的大 retained payload 会进入 `retainMessages` map；1GiB heap 复测中 260 条消息触发 OOM。
...
| --- | --- |
| 项目 | `quickmsg__smqtt` |
| 本地真阳性 ID | `SMQTT-APP-STATIC-0002` |
| 动态批次 | `p0` |
| 动态状态 | `verified_oom` |
...
- 触发方式：连接默认 MQTT TCP 1883，发送 retain=true、topic 唯一、payload 较大的 PUBLISH。
- 根因摘要：单条消息受 messageMaxSize 限制，但 retained topic 总数和总字节数无默认上限。
- 利用条件：匿名 publish 被允许；retained messages 功能启用；没有 retained count/bytes quota。
- 静态 source：`MQTT PUBLISH with retain=true, attacker-controlled distinct topicName and payload`
- 静态 sink：`PublishProtocol.filterRetainMessage -> DefaultMessageRegistry.retainMessages.put(topic, RetainMessage)`
- 攻击者驱动变量：`distinct retained topic cardinality and retained payload bytes up to the configured MQTT frame limit`
...
| 请求数 | `260` |
| 1GiB 严格证据 | `1g` |
| 原始日志路径 | `results/applications_dynamic_validation/p0/logs/SMQTT-APP-STATIC-0002.log` |
