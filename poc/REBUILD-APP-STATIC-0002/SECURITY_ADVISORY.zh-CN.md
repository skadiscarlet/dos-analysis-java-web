# Security Advisory PoC：Rebuild 匿名 barcode render 文本长度可触发图片渲染 OOM

## 摘要

Rebuild 匿名 `/commons/barcode/render` 会根据请求文本渲染 Code128 barcode。1GiB heap 复测中，较长文本触发图片/编码渲染大对象分配并 OOM。
...
| 本地真阳性 ID | `REBUILD-APP-STATIC-0002` |
| 动态批次 | `p1` |
| 动态状态 | `verified_oom` |
| 入口 | `/commons/barcode/render` |
| 权限前置 | 匿名或默认开放协议客户端 |
| 建议 CVSS 3.1 | `CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:H` |

## 技术细节

- 触发方式：GET 或 POST `/commons/barcode/render`，传入逐步增大的文本参数。
- 根因摘要：barcode 渲染前未对文本长度、输出像素或编码复杂度设置足够低的默认上限。
- 利用条件：匿名 barcode endpoint 暴露；请求行/header 限制仍允许达到触发阈值。
- 静态 source：`anonymous query parameters t and w`
- 静态 sink：`BarCodeSupport.createBarCodeImage computes Code128 auto width from new Code128Writer().encode(content).length and then calls MultiFormatWriter.encode, followed by MatrixToImageWriter.toBufferedImage`
- 攻击者驱动变量：`attacker-controlled content length and height parameter; width is recomputed from encoded content length after the fixed 1200 width cap, so long t can drive large matrix/image allocation`
...
| 请求数 | `3` |
| 1GiB 严格证据 | `1g` |
| 原始日志路径 | `results/applications_dynamic_validation/p1/logs/REBUILD-APP-STATIC-0002.log` |

证据摘要：

```text
匿名 Code128 barcode render，文本长度受默认请求行/header 限制；成功条件仍是目标 JVM OOM。
```
