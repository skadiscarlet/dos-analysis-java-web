# Security Advisory PoC：Erupt captcha height 参数可驱动 BufferedImage 大对象 OOM

## 摘要

Erupt sample 默认 H2 启动下，匿名 `/erupt-api/code-img` 接口的 height 参数可直接影响验证码图片高度。1GiB heap 复测中，极大 height 值触发 `BufferedImage` 分配并 OOM。
...
| --- | --- |
| 项目 | `erupts__erupt` |
| 本地真阳性 ID | `ERUPT-APP-STATIC-0001` |
| 动态批次 | `p1` |
| 动态状态 | `verified_oom` |
| 入口 | `/erupt-api/code-img` |
| 权限前置 | 匿名或默认开放协议客户端 |
| 建议 CVSS 3.1 | `CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:H` |

## 技术细节

- 触发方式：GET `/erupt-api/code-img?mark=<x>&height=<large>`，逐步增大 height。
- 根因摘要：height 未被限制在正常验证码范围内，传入 `new SpecCaptcha(150, height, 4)` 后创建巨大 `BufferedImage`。
- 利用条件：匿名 captcha endpoint 暴露；未对 height/总像素数做上限校验。
- 静态 source：`GET /erupt-api/code-img?mark=...&height=...`
- 静态 sink：`EruptUserController.createCode -> new SpecCaptcha(150, height, 4) -> captcha.out(); EasyCaptcha SpecCaptcha.graphicsImage creates BufferedImage(width,height,TYPE_INT_RGB)`
- 攻击者驱动变量：`attacker-controlled height query parameter`
...
| 请求数 | `3` |
| 1GiB 严格证据 | `1g` |
| 原始日志路径 | `results/applications_dynamic_validation/p1/logs/ERUPT-APP-STATIC-0001.log` |

证据摘要：

```text
Erupt sample 默认 H2 启动，匿名 captcha height 直接驱动 BufferedImage 分配。
```
