...
- 应用级严格 1G 堆二值真相：`results/applications_dynamic_validation/binary_truth_collection.json`、`results/applications_dynamic_validation/BINARY_TRUTH_COLLECTION.md`
...
应用级 P0 动态验证使用 `scripts/run_application_p0_dynamic_validation.py`，以 `results/applications_static_analysis/_static_validation/all_candidates_dynamic_validation.md` 的 P0 队列为输入，输出 `summary.json`、`summary.csv`、`findings.jsonl`、`P0_DYNAMIC_VALIDATION_REPORT.md` 和原始日志到 `results/applications_dynamic_validation/p0/`。当前严格 1G 堆冻结口径为：15 个候选中 8 个真实触发 OOM 真阳性，5 个已执行但未在 1G 堆下确认 OOM，2 个因默认应用环境、登录态或特殊 agent/compose 前置条件未满足而保持环境阻塞；真阳性证据必须来自外部协议/HTTP 请求触发目标 JVM 的 `OutOfMemoryError`，且显式 JVM heap / `-Xmx` 至少为 1GiB。小堆 OOM 可保留为弱证据，但不计入真阳性统计。

应用级 P1 动态验证使用 `scripts/run_application_p1_dynamic_validation.py`，以 `results/applications_static_analysis/_static_validation/all_candidates_dynamic_validation.md` 的 P1 队列为输入，输出 `summary.json`、`summary.csv`、`findings.jsonl`、`P1_DYNAMIC_VALIDATION_REPORT.md` 和原始日志到 `results/applications_dynamic_validation/p1/`。当前严格 1G 堆冻结口径为：89 个候选中 3 个真实触发 OOM 真阳性，32 个已执行但未在 1G 堆下确认 OOM，0 个保持 `probe_error`，54 个因默认服务环境、业务数据、登录态、agent/compose 或协议 harness 前置条件未满足而保持 `precondition_blocked`；其中第一优先级补测 38 个候选，2 个真实 OOM、25 个已执行未确认 OOM、0 个探针错误、11 个前置条件阻塞；真阳性证据必须来自外部协议/HTTP 请求触发目标 JVM 的 `OutOfMemoryError`，且显式 JVM heap / `-Xmx` 至少为 1GiB。

应用级 P2 动态验证使用 `scripts/run_application_p2_dynamic_validation.py`，以 `results/applications_static_analysis/_static_validation/p2_dynamic_validation_triage.md` 为输入，输出 `summary.json`、`summary.csv`、`findings.jsonl`、`P2_DYNAMIC_VALIDATION_REPORT.md` 和原始日志到 `results/applications_dynamic_validation/p2/`。当前严格 1G 堆冻结口径为：34 个候选中 0 个真实触发 OOM 或持续服务不可用真阳性，1 个已执行但未在 1G 堆下确认 OOM/不可用，7 个 triage 推荐候选因默认服务环境、业务数据、登录态或依赖栈未补齐保持 `precondition_blocked`，26 个按 P2 triage 保持 `triage_not_selected`；真阳性证据必须来自外部协议/HTTP 请求触发目标 JVM 的 `OutOfMemoryError`、GC death、线程/连接池耗尽或持续服务不可用，且显式 JVM heap / `-Xmx` 至少为 1GiB。

应用级二值真相集合由 `scripts/collect_dynamic_validation_binary_truth.py` 从 `p0`、`p1`、`p2`、`new`、`new_retest` 五个批次归并生成。当前严格 1G 堆口径为：`13` 条 `confirmed_true_positive`、`176` 条 `unconfirmed_or_non_oom`。只有 confirmed target failure 且显式 JVM heap / `-Xmx` 至少 1GiB 的记录可计为真阳性；小堆 OOM 和仅有 container memory limit 的证据统一归入未确认或非 OOM。
