...
predicate flowRow(Method source, Parameter input, Element sinkSite, string target, string sinkText,
  ...
  or exists(DataFlow::Node sourceNode, DataFlow::Node sinkNode, MethodCall append, Parameter request |
    EntryToGrowthFlow::flow(sourceNode, sinkNode) and attackerSourceNode(source, input, sourceNode) and
    servletRequestStringMaterialization(append, request) and sinkNode.asParameter() = request and
    callsWithin(source, append.getEnclosingCallable(), _) and sinkSite = append and target = "size" and
    sinkText = append.getArgument(0).toString() and
    path = source.getQualifiedName() + ">" + append.getEnclosingCallable().getQualifiedName() and
    phase = "entry>global_dataflow>growth" and
    (
      source = append.getEnclosingCallable() and confidence = "proven" and coverage = "complete" and
      note = "same_handler_global_dataflow"
      or source != append.getEnclosingCallable() and confidence = "partial" and coverage = "partial" and
      note = "transitive_callgraph_witness_requires_path_coverage"
    )
  )
  or exists(Expr sink, Expr demand, Method serviceTarget, string route |
    ...
    growthDemand(sink, target, demand) and callsWithin(serviceTarget, sink.getEnclosingCallable(), _) and
    sinkSite = sink and sinkText = demand.toString() and
    path = source.getQualifiedName() + "[" + route + "]>" + serviceTarget.getQualifiedName() + ">" +
      sink.getEnclosingCallable().getQualifiedName() and
