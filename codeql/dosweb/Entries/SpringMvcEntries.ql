...
string getMappingPath(Annotation annotation) {
  ...
  not exists(string value |
    value = annotation.getAStringArrayValue("value") or
    value = annotation.getAStringArrayValue("path")
  )
}

string sourceStringDefault(Field field) {
  exists(CompileTimeConstantExpr initializer |
    initializer = field.getInitializer() and result = initializer.getStringValue()
  )
  or exists(Field referenced, CompileTimeConstantExpr initializer |
    field.getInitializer().(VarAccess).getVariable() = referenced and
    initializer = referenced.getInitializer() and result = initializer.getStringValue()
  )
}

string getResolvedSpelDefault(Annotation annotation) {
  exists(string raw, string prefix, Field field |
    raw = getMappingPath(annotation) and (raw.matches("#{%}") or raw.matches("${%}")) and
    raw = "#{" + prefix + "." + field.getName() + "}" and result = sourceStringDefault(field) and
    not exists(Field other, string otherValue |
      other != field and raw = "#{" + prefix + "." + other.getName() + "}" and
      otherValue = sourceStringDefault(other) and otherValue != result
    )
  )
}

string getResolvedSpelClassDefault(Method method) {
  exists(Annotation annotation |
    ...
    result = getResolvedSpelDefault(annotation)
  )
}
...
predicate springRow(
  ...
  exists(Method method, Annotation mapping, Parameter parameter, string classPath, string methodPath, string verb |
    ...
    not classPath.matches("#{%}") and not classPath.matches("${%}") and
  ...
  or exists(Method method, Annotation mapping, Parameter parameter, string resolvedPath, string verb |
    ...
    coverageNote = "spring_spel_route_default_requires_runtime_binding"
  )
  or exists(Method method, Annotation mapping, Parameter parameter, string resolvedClassPath, string methodPath, string joinedPath, string verb |
    ...
    authContext = "unknown" and inputName = parameter.getName() and inputType = parameter.getType().toString() and
    materializationPhase = "unknown" and coverageStatus = "partial" and
    coverageNote = "spring_spel_class_route_default_requires_runtime_binding"
  )
  or exists(Method method, Annotation mapping, Parameter parameter, Method register, Expr annotatedService |
    ...
    isSourceMethod(method) and isSourceExpr(mapping) and isSourceExpr(annotatedService) and
