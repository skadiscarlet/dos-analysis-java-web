...
class EntrySecurityFact:
    entry_id: str; kind: Literal["annotation", "filter", "servlet_constraint", "netty_gate", "mqtt_gate", "configuration", "dependency_coverage"]; location: str; line: int; value: str; coverage: Literal["complete", "partial", "unsupported"]; fact_id: str = ""
    def __post_init__(self) -> None:
        _id(self.entry_id, "entry:"); _str(self.location); _str(self.value)
        if self.kind not in {"annotation", "filter", "servlet_constraint", "netty_gate", "mqtt_gate", "configuration", "dependency_coverage"} or self.coverage not in {"complete", "partial", "unsupported"} or not isinstance(self.line, int) or isinstance(self.line, bool) or self.line < 0: _bad()
    ...
    def from_dict(cls,r:Mapping[str,object])->"EntrySecurityFact":
        ...
        return cls(cast(str,r["entry_id"]),cast(Literal["annotation","filter","servlet_constraint","netty_gate","mqtt_gate","configuration","dependency_coverage"],r["kind"]),cast(str,r["location"]),cast(int,r["line"]),cast(str,r["value"]),cast(Literal["complete","partial","unsupported"],r["coverage"]),cast(str,r["fact_id"]))
...
class ReachabilityDecision:
    entry_id:str; auth_contract_id:str; auth_context:Literal["unauthenticated","low_privilege","privileged","unknown"]; status:Literal["ordinary_attacker_reachable","not_entry_reachable","unknown"]; evidence_ids:tuple[str,...]; decision_id:str=""
    def __post_init__(self)->None:
        _id(self.entry_id,"entry:"); _id(self.auth_contract_id,"auth_contract:")
        if self.auth_context not in {"unauthenticated","low_privilege","privileged","unknown"} or self.status not in {"ordinary_attacker_reachable","not_entry_reachable","unknown"} or len(self.evidence_ids)>32: _bad()
        if (self.auth_context in {"unauthenticated","low_privilege"}) != (self.status=="ordinary_attacker_reachable") and not (self.auth_context=="unknown" and self.status=="unknown") and not (self.auth_context=="privileged" and self.status=="not_entry_reachable"): _bad()
        for x in self.evidence_ids:_id(x,"security:")
    ...
    def semantic_identity(self)->dict[str,object]:return {"entry_id":self.entry_id,"auth_contract_id":self.auth_contract_id,"auth_context":self.auth_context,"status":self.status,"evidence_ids":list(self.evidence_ids)}
