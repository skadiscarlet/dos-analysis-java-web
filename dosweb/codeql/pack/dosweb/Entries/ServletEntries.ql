...
predicate servletRow(
  ...
  or exists(Method callback, MethodCall setFilter, MethodCall urls, Parameter request |
    ...
    urls.getQualifier() = setFilter.getQualifier() and staticUrlPattern(urls, routeOrEvent) and framework = "servlet" and protocol = "http" and
  ...
  or exists(MethodCall addServlet, Method enclosing |
    addServlet.getMethod().getName() = "addServlet" and isContextHandlerType(addServlet.getMethod().getDeclaringType()) and
    enclosing = addServlet.getEnclosingCallable() and not exists(Type handlerType | holderBinds(addServlet, handlerType)) and framework = "servlet" and protocol = "http" and
