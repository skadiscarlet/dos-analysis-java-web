...
predicate getPath(Annotation annotation, string path) {
  isPathAnnotation(annotation) and path = annotation.getStringValue("value")
}
...
predicate jaxRsRow(
  ...
  exists(Method method, Parameter parameter, MethodCall registration, Type resource, string classPath, string methodPath, string verb |
    ...
    registrationBinds(registration, resource) and
  ...
  or exists(Method method, Annotation mapping, Parameter parameter |
    ...
    not exists(MethodCall registration | registrationBinds(registration, method.getDeclaringType())) and
    ...
    registrationLine = mapping.getLocation().getStartLine() and routeOrEvent = "unresolved_jax_rs_resource" and authContext = "unknown" and
