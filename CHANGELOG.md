...
- 将 RC1 backend execution 收窄到正式 Flow 域中实际可消费 accepted-task population 性质的 `async_work_growth` / `tasks` 候选：普通 bytes、entries、connections 等 Growth 不再触发无关的全项目 allocation/CFG 查询，也不会被无关 RC1 source/backend gap 污染。正式 Entry/Growth/Flow 仍先完成并经校验；存在相关 task 候选时 full 与 propagation-off 消融继续执行完全相同的 provider/raw facts，仅在消费侧分叉。该修复关闭 PoC-33 大库第一条 RC1 query 产生 281–857 MB decoded JSON、超过通用 64 MiB 安全上限且原错误被 cleanup 遮蔽的问题，而不提高通用 artifact/decoded 上限。
...
- 将 production RC1 内部 selected-query 的显式 CodeQL 上限从 300 秒提高到 900 秒：PoC-33 最大 Ruoyi database 的 `ResourceLifecycleTaskRelations` 全程序 join 在 300 秒处稳定被终止并留下受控 `.pending` quarantine；新上限仍为有界执行，超时继续作为 terminal selected-query failure，不降级、不跳过、不发布 partial lifecycle 结论。
- 修复 production lifecycle 将完整 RC1 facts/solver state 重新编码为单个 stage artifact 的实现偏差：raw backend state 按原有 in-memory provider 合同只作本地临时证据，正式归档发布 compact path-bound bindings、facts/result hash、query/database/source/implementation provenance 与 lifecycle evidence；不再因大型 Ruoyi facts 展开超过 16 MiB artifact 上限而中止。P0 聚合契约同步移除两项并非所有 coverage 路径都会生成的 raw monolith。
- 将标准 real-provider full 命令的默认 target attempts 继续固定为 2，同时允许修复型归档在 64 次硬上限内显式声明并验证实际 attempt 上限；offline evaluator 只能从已完成、run-id 匹配的 acceptance manifest 读取该值。这样保留 provider/实现故障的真实重试历史，不重写 state，也不把失败 attempt 当作完成目标。
- 新增正式 `growth_static_facts.jsonl`，持久化每个 bounded slice 实际交给 Growth Contract 分类器并由本地 verifier 消费的 typed facts，按 `growth_id` 绑定稳定 fact identity；production 发布前与 P0 离线聚合均据此验证 attacker influence 和 required static evidence，修复 contract 仅在运行时可验证、归档后被误判为 dangling reference 的缺口。Growth implementation identity 升至 v33，强制已有 full 从 Growth 起重算下游 stages。
- batch runner 新增显式 `--refresh-completed`：重新进入 authoritative completed targets，但仍由 pipeline resume 按 implementation/upstream fingerprint 复用或失效 stages，并受原有 16 次 attempt 硬上限约束；用于 schema/implementation 升级后的同 plan 修复，不重建 plan、不清空历史 attempt。
