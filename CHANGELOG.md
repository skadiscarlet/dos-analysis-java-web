...
- RC1 的两条 lifecycle queries 改用各自独立的私有 publication workspace，避免大型连续查询复用同一 `.generations` 目录时在第二条结果发布后稳定触发 generation-directory release failure；query suite identity、顺序和 provenance 保持不变。
