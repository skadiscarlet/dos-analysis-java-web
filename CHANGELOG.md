...
- production RC1 的两条 lifecycle queries 改走正式 `_CodeqlQueryWorkspace` ownership/consume/cleanup 合同，再把经过 binding 校验的 payload snapshots 交给 in-memory adapter/solver；不再使用 direct `run_query` 的自动 publication release 路径。standalone CLI 继续使用 direct 路径；query suite identity、顺序、digest provenance 和 private database binding 保持不变。
